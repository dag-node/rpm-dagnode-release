#!/usr/bin/env bash
# Sign the dagnode-release RPMs with the dag-node org signing key and export the matching public
# key. Runs INSIDE the matching-EL build container (rockylinux:elN) so the rpm/gnupg toolchain
# that signs is the same one that verifies on the target host (EL10 uses rpm-sequoia) -- see the
# org playbook in github-org-dag-node/org/GPG-HINTS.md. The CI workflow invokes it over the freshly
# built RPMs before publishing, so every published package carries a header signature an operator
...
# passes a silent rpmsign no-op. Errors use the ::error:: prefix so GitHub Actions surfaces them
# as annotations; the text reads plainly on a local terminal too. Every secret (imported private
# key, passphrase) lives in a tmpfs (RAM) scratch tree wiped on exit -- never persistent disk,
# never the container's real keyring or rpmdb.
set -euo pipefail
...
# into the override signs nothing; here %{__gpg} stands alone.
write_rpm_macros() {
